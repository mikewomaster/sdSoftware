namespace HslCommunication.Robot.EFORT
{
    ...
    public class ER7BC10 : NetworkDoubleBase, IRobotNet
    {
        ...
        {
            byte[] array = new byte[0x26];
            Encoding.ASCII.GetBytes("MessageHead").CopyTo(array, 0);
            BitConverter.GetBytes((ushort) array.Length).CopyTo(array, 0x10);
            BitConverter.GetBytes((ushort) 0x3e9).CopyTo(array, 0x12);
            BitConverter.GetBytes((ushort) this.softIncrementCount.GetCurrentValue()).CopyTo(array, 20);
            Encoding.ASCII.GetBytes("MessageTail").CopyTo(array, 0x16);
            return array;
        }
        ...
        public OperateResult<EfortData> ReadEfortData()
        {
            ...
            return EfortData.PraseFrom(result.Content);
        }
        ...
        {
            return string.Format("ER7BC10 Robot[{0}:{1}]", this.IpAddress, this.Port);
        }
